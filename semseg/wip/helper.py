...
# Store results for each image with a unique row ID
objects_dict = {}

# Process each image and store the objects for each row ID
image_folder = "path/to/folder/with/predictions"  # Folder with segmented images
...
for image_path in segmented_image_paths:
    # Get the filename without the extension and omit "gtFine" and "labelColors"
    filename = os.path.splitext(os.path.basename(image_path))[0]
    polygon_data = image_to_polygon_format(image_path)
    objects_dict[filename] = polygon_data  # Use modified filename as the row_id_column_name
